...
localStorage.setItem("selectedTheme", JSON.stringify({"id": 3, "text": "Indigo", "bcgColor": "#3F51B5", "fontColor": "#FAFAFA"}));
...
$(document).ready(function () {
    ...
    // ambil value dari elemen select .my-select
        var theme = JSON.parse(localStorage.getItem('themes'))[$('.my-select').val()];
    ...
    var current = JSON.parse(localStorage.selectedTheme);
    $("body").css({"backgroundColor":current.bcgColor});
    $(".text-center").css({"color": current.fontColor});
    //Change selection
